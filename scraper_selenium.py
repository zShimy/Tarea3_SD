...
try:
    while True:
        ...
        for request in nuevos_requests:
            try:
                ...
                if nuevos:
                    print(f"[NUEVO] {len(nuevos)} eventos nuevos desde: {request.url}")
                    collection.insert_many(nuevos)
                    uuids_existentes.update(a["uuid"] for a in nuevos)
                else:
                    print(f"[REPETIDO] Solo eventos repetidos desde: {request.url}")
    ...
    print("[FINALIZACION] Captura interrumpida por el usuario.")
finally:
    try:
        driver.quit()
    except Exception as e:
